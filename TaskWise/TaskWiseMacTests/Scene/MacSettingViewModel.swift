...
@Suite("MacSettingsViewModel", .tags(.viewModel))
final class MacSettingsViewModelTests {
    private var sut: MacSettingsViewModel!
    ...
    @Test("Close sheet")
    func closeSheet() {
        sut.newCategoryName = "test"
        sut.newColumnName = "test"
        sut.newPriorityName = "test"
        sut.isNewCategorySheetPresented = true
        sut.isNewColumnSheetPresented = true
        sut.isNewPrioritySheetPresented = true

        sut.closeSheet()

        #expect(
            sut.newCategoryName == .empty && sut.newColumnName == .empty && sut.newPriorityName == .empty &&
            !sut.isNewColumnSheetPresented && !sut.isNewCategorySheetPresented && !sut.isNewPrioritySheetPresented
        )
    }
}
